...
template <class data_t, template <typename> class vars_t,
          ...
          template <typename> class rhs_vars_t>
Discriminant<data_t>
FourDerivScalarTensor<coupling_and_potential_t>::compute_discriminant(
    const rhs_vars_t<data_t> &rhs_vars, const vars_t<data_t> &vars,
    const vars_t<Tensor<1, data_t>> &d1,
    const diff2_vars_t<Tensor<2, data_t>> &d2, const vars_t<data_t> &advec,
    const Coordinates<data_t> &coords) const
{
    ...
    //     FOR(i)
    //     {
    //         Ci[i] = -4. * dfdphi * (d1.Pi[i] + vars.K * d1.phi[i] /
    //         (double)GR_SPACEDIM) - 4. * d2fdphi2 * vars.Pi * d1.phi[i];
    //         FOR(j, k) { Ci[i] += - 4. * dfdphi * h_UU[j][k] * d1.phi[k] *
    //         vars.A[i][j]; }
    //     }
    ...
    //     }
    //     data_t Cnn = 4. * dfdphi / lapse_regularised * (rhs_vars.Pi -
    //     advec.Pi -
    //         vars.chi * compute_dot_product(d1.phi, d1.lapse, h_UU)) +
