    ...
    BoxLoops::loop(
        ...
        if (m_level == min_level) //m_level == min_level
        {
          ...
          //             m_state_new, m_state_new, INCLUDE_GHOST_CELLS);
            ...
            if (first_step)
            {
                constraints_file.write_header_line({"<chi>", "<rho>", "L2_Ham", "L2_Mom", "Vol_osc", "M_osc", "rho_max", "wcc_max", "<rho_phi>", "<rho_GB>", "ln(a)"});
            }
            // if (m_level == min_level){
            constraints_file.write_time_data_line({chi_mean, m_bh_amr.m_rho_mean, L2_Ham, L2_Mom, vol_obj, mass_obj, m_bh_amr.m_rho_max, wcc_max, rho_phi_mean, rho_GB_mean, lna});
            // }
        //Custom Extaction
