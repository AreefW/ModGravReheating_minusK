...
// class Excision95Density
{
  ...
  public:
    Excision95Density(const double a_dx, const std::array<double, CH_SPACEDIM> a_obj_center,
                        const double a_obj_rho_max, const double a_obj_bound_cutoff, const double a_rho_mean, const double a_obj_rad)
        : m_dx(a_dx), m_deriv(m_dx), m_obj_center(a_obj_center), m_obj_rho_max(a_obj_rho_max), m_obj_bound_cutoff(a_obj_bound_cutoff), m_rho_mean(a_rho_mean), m_obj_rad(a_obj_rad)
    {
    }
