...
void KerrBH4dSTLevel::postRestart()
{
    ...
    //     AMRReductions<VariableType::diagnostic>
    //     amr_reductions_diag(m_bh_amr);
    //         double phys_vol = amr_reductions_diag.sum(c_sqrt_gam);
    //         //m_bh_amr.m_rho_mean = amr_reductions_diag.sum(c_rho_phi) /
    //         phys_vol; m_bh_amr.m_rho_mean =
    //         amr_reductions_diag.sum(c_rho_scaled) / phys_vol;
    //         // double rho_mean_all = (amr_reductions_diag.sum(c_rho_phi) +
    //         amr_reductions_diag.sum(c_rho_g2) +
    //         amr_reductions_diag.sum(c_rho_g3) +
    //         amr_reductions_diag.sum(c_rho_GB)) / phys_vol;
    //         // m_bh_amr.m_K_mean = - sqrt(3.0 * m_bh_amr.m_rho_mean);
    //         m_bh_amr.m_K_mean = amr_reductions_diag.sum(c_K_scaled) /
    //         phys_vol; m_bh_amr.m_rho_max =
    //         amr_reductions_diag.max(c_rho_scaled); m_bh_amr.m_S_mean =
    //         amr_reductions_diag.sum(c_S_scaled) / phys_vol;

    //     // pout() << "Calculated K mean as " << m_bh_amr.m_K_mean
    //     //        << " at t = " << m_time << " on restart at level " <<
    //     m_level
    //     //        << endl;
    //     // pout() << "PostRestart c_rho_contrast = " <<
    //     amr_reductions_diag.sum(c_rho_contrast) << endl;
    //      pout() << "rho phi = " << amr_reductions_diag.sum(c_rho_phi) /
    //      phys_vol << endl; pout() << "rho g2 = " <<
    //      amr_reductions_diag.sum(c_rho_g2) / phys_vol << endl; pout() << "rho
    //      g3 = " << amr_reductions_diag.sum(c_rho_g3) / phys_vol << endl;
    //      pout() << "rho GB = " << amr_reductions_diag.sum(c_rho_GB) /
    //      phys_vol << endl; pout() << "rho mean all = " << m_bh_amr.m_rho_mean
    //      << endl; pout() << "phys_vol = " << phys_vol << endl; pout() << "K
    //      mean = " << m_bh_amr.m_K_mean << endl; pout() << "rho max = " <<
    //      m_bh_amr.m_rho_max << endl;
    // }
}
...
{
    BoxLoops::loop(HamTaggingCriterion(m_dx, m_p.center_obj, m_p.obj_r),
                   current_state_diagnostics, tagging_criterion);

    // std::array<double, CH_SPACEDIM> center_osc = {33.1562, 18.0938, 33.2188};
    // // center of oscillon double Lregrid = 32.0; BoxLoops::loop(
