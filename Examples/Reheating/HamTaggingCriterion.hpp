...
class HamTaggingCriterion
{
  protected:
    const double m_dx;
    std::array<double, CH_SPACEDIM> m_center_BS;
    double m_rad;

  public:
    HamTaggingCriterion(double dx, std::array<double, CH_SPACEDIM> center_BS,
                        double rad)
        : m_dx(dx), m_center_BS(center_BS), m_rad(rad){};
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        const Coordinates<data_t> coords(current_cell, m_dx, m_center_BS);
