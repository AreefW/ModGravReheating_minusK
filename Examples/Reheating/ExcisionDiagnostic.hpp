...
template <class theory_t> class Excision95Density
{
  protected:
    ...
    const std::array<double, CH_SPACEDIM>
        m_center; //!< The grid center >> calculate r from origin
    const FourthOrderDerivatives m_deriv;
  ...
  public:
    ...
    void compute(const Cell<double> current_cell) const
    {
        ...
        else
        {
            ...
            //   std::array<double, CH_SPACEDIM> pos_rho_max = {coords.x,
            //   coords.y, coords.z}; double rad_rho_max = coords.get_radius();
            //   pout() << "Stored position: " <<
            //         "(" << pos_rho_max[0] << "," << pos_rho_max[1] << "," <<
            //         pos_rho_max[2] << ")"
            //         << " r: " << rad_rho_max << std::endl;
            //   Coordinates<double> coords_obj{current_cell, m_dx,
            //   pos_rho_max};
            // }
