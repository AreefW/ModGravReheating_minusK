...
void KerrBH4dSTLevel::prePlotLevel()
{
    ...
    BoxLoops::loop(
        Excision95Density<FourDerivScalarTensorWithCouplingAndPotential>(
            m_dx, m_p.center, m_bh_amr.m_rho_max, m_p.obj_bound_cutoff,
            m_bh_amr.m_rho_mean),
...
{
    BoxLoops::loop(HamTaggingCriterion(m_dx, m_p.center_BS, m_p.rad),
    ...
    if (calculate_diagnostics)
    {
        ...
        if (m_level == min_level) // m_level == min_level
        {
            ...
            */
            ...
            // m_bh_amr.m_S_mean = amr_reductions_diag.sum(c_S_scaled) /
            // phys_vol;
            m_bh_amr.m_K_mean = -sqrt(3.0 * m_bh_amr.m_rho_mean);
            ...
            //    {33.25, 18.25, 33.25}; // specified point CustomExtraction
            //    extraction(c_rho_scaled, num_points, m_p.L, extr_point,
            //                               m_dt, m_time);
            //    extraction.execute_query(&interpolator, m_p.data_path +
            //    "rho_dat");
            //----
        }
    }
