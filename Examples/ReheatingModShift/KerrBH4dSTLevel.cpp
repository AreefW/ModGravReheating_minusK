...
void KerrBH4dSTLevel::postRestart()
{
    ...
    if (m_time == 0.0)
    {
        ...
        m_bh_amr.m_rho_max = amr_reductions_diag.max(c_rho_total);
        ...
        pout() << "Compute Gauge and calculate RHS diagnostics..." << endl;
        ...
        RHSDiagnostics<FourDerivScalarTensorWithCouplingAndPotential,
            ...
            rhs_diagnostics(fdst, m_p.modified_ccz4_params,
                            ...
                            m_bh_amr.m_K_mean, m_bh_amr.m_rho_mean, m_p.center,
                            m_p.G_Newton);
        BoxLoops::loop(rhs_diagnostics, m_state_new, m_state_diagnostics,
                       EXCLUDE_GHOST_CELLS);
    ...
    if (calculate_diagnostics)
    {
        ...
        if (m_level == min_level) // m_level == min_level
        {
            AMRReductions<VariableType::diagnostic> amr_reductions_diag(
                m_bh_amr);
            double phys_vol = amr_reductions_diag.sum(c_sqrt_gam);
            double phys_vol_scaled = amr_reductions_diag.sum(c_sqrt_gam) / m_p.L;
            ...
            double rho_phi_mean = amr_reductions_diag.sum(c_rho_phi_scaled) / phys_vol;
            double rho_GB_mean = amr_reductions_diag.sum(c_rho_GB_scaled) / phys_vol;
            // m_bh_amr.m_rho_max = amr_reductions_diag.max(c_rho_total);
            ...
            // double rho_max = amr_reductions_diag.max(c_rho_scaled);
            double rho_max = amr_reductions_diag.max(c_rho_total);
            m_bh_amr.m_rho_max = rho_max;
            /*
            if (last_step){
            ...
            */
            ...
            double max_Lm1 = amr_reductions_diag.max(c_Lm1);
            double max_RGBterm = amr_reductions_diag.max(c_RGBterm);
            double max_ricci_term = amr_reductions_diag.max(c_ricci_term);
            double max_sqrt_V = amr_reductions_diag.max(c_sqrt_V);
            double max_nabla_phi = amr_reductions_diag.max(c_nabla_phi);
            double max_nabla2_phi = amr_reductions_diag.max(c_nabla2_phi);
            double max_abs_dfdphi = amr_reductions_diag.max(c_absdfdphi);

            //----
            AMRReductions<VariableType::evolution> amr_reductions_evo(m_bh_amr);

            double chi_mean = amr_reductions_evo.sum(c_chi) / pow(m_p.L, 3.); // / phys_vol_scaled;
            ...
            if (first_step)
            {
                constraints_file.write_header_line(
                    {"<chi>", "<rho>", "L2_Ham", "L2_Mom", "Vol_osc", "M_osc",
                     ...
                     "rho_max_GB", 
                     //"rho_max_GB_scaled",
                     "max_Lm1", "RGBterm", "max_ricci_term", "max_sqrt_V", "max_nabla_phi",
                     "max_nabla2_phi", "max_abs_dfdphi", "phys_vol"});
            }
            ...
            constraints_file.write_time_data_line(
                {chi_mean, m_bh_amr.m_rho_mean, L2_Ham, L2_Mom, vol_obj,
                 ...
                 rho_max_GB, 
                 // rho_max_GB_scaled,
                 max_Lm1, max_RGBterm, max_ricci_term, max_sqrt_V, max_nabla_phi,
                 max_nabla2_phi, max_abs_dfdphi, phys_vol});
            // }
            // Custom Extaction
